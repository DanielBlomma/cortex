...
function finalizeImpactResult(result: ImpactResult, includeScores: boolean): Record<string, unknown> {
  ...
  const { impact_score, profile_score, note_score, semantic_score, graph_score, trust_score, ...publicResult } =
    result;
  void impact_score;
  void profile_score;
  void note_score;
...
export function buildImpactResults(params: {
  ...
  const results = [...params.visited.entries()]
    ...
    .map(([id, metadata]) => {
      ...
      return {
        ...
        path: entity?.path || String(catalogEntry.path ?? "") || undefined,
        hops: metadata.hops,
