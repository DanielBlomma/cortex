...
case "$SUBCOMMAND" in
  ...
  event)
    ...
    if [[ -z "$EVENT_NAME" ]]; then
      echo "Usage: ./scripts/plan-state.sh event <command>"
      exit 1
    fi
    mkdir -p "$PLAN_DIR"
  ...
  todo)
    ...
    case "$TODO_ACTION" in
      ...
      remove)
        ...
        ;;
      *)
        echo "Usage: ./scripts/plan-state.sh todo <add|list|done|reopen|remove> [args]"
        exit 1
        ;;
    esac
    ;;
  *)
    echo "Usage: ./scripts/plan-state.sh [show|reset|ensure|event <command>|todo <...>]"
    exit 1
    ;;
esac
