...
function classifyDialectNode(node) {
  ...
  if (node.type === "impl_item") facts.push({ category: "declaration_structure", kind: "type", form: "declaration" });
  if (node.type === "attribute_item") {
    const attribute = node.text.replace(/^#\s*!?\s*\[/, "").replace(/\]\s*$/, "").trim();
    if (attribute === "test") facts.push({ category: "test_shape", kind: "test_declaration", form: "attribute" });
  }
  return facts;
}
