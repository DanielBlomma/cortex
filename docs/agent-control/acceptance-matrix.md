...
| REQ-6 | Keep agent/process control useful without slowing small maintenance PRs. | WO-056-062 prove semantics, persistence, trusted reads/appends, canonical Current State, and exact-HEAD Git-anchored initial provisioning through control `98301f8`. | Packet 077 relaxes only the `.agents` parent permission from exact-private to stable physical owner-controlled and non-group/world-writable, while task/stage privacy, exact target inventory, sibling neutrality, atomic publication, and every seed/Git/source contract remain exact. Public writing/rendering, manager/handoff replacement, auto-emission, broader dogfood, and WO-055 remain separate decisions. | Hermetic tracked `.agents/skills` coverage proves positive mode `0755`, negative writable/symlink/type/race cases, sibling byte/mode/identity neutrality, unchanged API/results, retry/concurrency/failure behavior, and the full WO-062 MCP/root/package/ownership/containment matrix plus combined review. | Git-anchored provisioning accepted at feature `769396a`/control `98301f8`; shared-parent compatibility accepted at `32d1f7d`, merged as `ed399e1`, and released in `v2.7.0`; public mutation/dogfood gates and WO-055 remain unapproved |
...
| REQ-18 | Give agents task-conditioned evidence about how the local codebase normally structures and implements comparable solutions, not merely context about symbols or formatting. | Packet 054 defines codebase dialect as recurring, locally evidenced structural and implementation patterns. WO-051-054 are accepted. WO-056/057 preserve viable SQL-002/TypeScript-002 bindings as current derived facts in deterministic replayable state while retaining incomplete VB6/VB.NET availability and six active blockers. | Keep WO-055 suspended. Any later resumption requires a separate decision from maintained current state; two viable bindings cannot imply whole-WO viability, review readiness, or acceptance. | Original 14/14 availability and blind quality gates remain unchanged; oracle/native parity is 19/19 each and the default-off persistence/workflow boundary passed the full WO-057 acceptance suite. | Open and suspended before blind evidence; maintained-state Stage 1 accepted at `180b122` |
| REQ-19 | Make Cortex an installable DeepSeek Harness context-retrieval capability without forking Harness, duplicating ranking logic, or weakening local/project isolation. | WO-DSH-056 pins Harness `0.1.1-rc.2` and rejects the unsafe process-global MCP topology. Accepted WO-DSH-057 implements the native agent-scoped provider, four CLI-backed tools, five scoped skills, bounded managed subprocess and validated envelopes as `@danielblomma/dsh-cortex`; accepted WO-DSH-058 independently verifies the exact dirty candidate with all six roles GO and findings `none`. | V1 is merged and released in Cortex `2.6.0`; the separately published bundle follows the accepted agent-scoped contract. V2 proactive retrieval remains separately authorized. | Upstream 18/18; focused compatibility/provider/manifest/skill tests 32/32; bundle 6/6; independent negative lifecycle 2/2; isolated roots 2/2; offline/PATH-less commands 4/4; root 490/490; MCP 426/426; audits 6/6 zero; two identical 12-entry packs with SHA-256 `d7be11a964d6aa4562fac22a3a944e1919a0b46652f3f1a2dc732dde96c90beb`; install 2/2, upgrade 2/2, headless help, Web HTTP 200, controlled shutdown, and removal 2/2. | Released in Cortex `2.6.0`; V2 remains separately gated |
