...
| R20-HARNESS | DeepSeek Harness integration | Developer-preview API drift, incorrect agent/workspace binding, or unbounded child output can expose stale or wrong-project source across concurrent Harness workspaces; later proactive context can also persist untrusted code, recurse, or exceed latency/context budgets. | WO-DSH-056 pins upstream and rejects the process-global MCP client. Accepted WO-DSH-057 derives authority only from exact `ToolRunContext.agent` and canonical `agent.session.header.cwd`, scopes tools/skills in `agent.ctx`, and executes the exact package-owned CLI through bounded/cancellable `ctx.subprocess`. V2 remains absent/default-off and requires a separate generation/provenance/replay/budget gate before any implementation. | WO-DSH-058 independently returned six-role GO with findings `none`: upstream 18/18; focused 32/32; bundle 6/6; negative lifecycle 2/2; roots 2/2; offline commands 4/4; root 490/490; MCP 426/426; six audits zero; identical 12-entry packs at SHA-256 `d7be11a964d6aa4562fac22a3a944e1919a0b46652f3f1a2dc732dde96c90beb`; pinned Web/headless lifecycle passed. Cortex 2.6.0 and the separate V1 bundle are published; V2 remains separately gated. | CLI and Runtime + Release and Distribution + Security/Privacy | Mitigated and released for V1; any V2 remains separately gated |
